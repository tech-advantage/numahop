(function () {
	...
	angular.module('numaHopApp.service')
		.factory('WorkflowHandleSrvc', function (codeSrvc, WorkflowSrvc) {
			...
			*/
			var stateStatusIsDone = function (status) {
				if (status === 'FINISHED' || status === 'CANCELED') {   //|| status === 'SKIPPED'
					return true;
				}
				return false;
			};
			...
			var isStatePendingOrSkipped = function (workflow, key) {
                ...
                _.chain(workflow.states)
                    ...
                    .each(function (state) {
                        if (stateStatusIsPending(state.status) 
                                || stateStatusIsSkipped(state.status)) {
                            result = true;
                        }
                    });
                return result;
			...
			var canConstatBeDeleted = function (workflow) {
				return !isConstatBeforeNumValidated(workflow)
				    && !isConstatConfirmed(workflow)
					&& !isConstatValidated(workflow)
					&& !isConstatAfterNumValidated(workflow);
			};
			...
			var canCondReportBeValidated = function (workflow) {
				return isStatePendingOrSkipped(workflow, "VALIDATION_CONSTAT_ETAT")
