(function () {
    ...
    angular.module('numaHopApp.controller').controller('ModalWithValidationCtrl', function ($uibModalInstance, $q, $scope, options) {
        ...
        $scope.ok = function () {
            if ($scope.validationFns.length > 0) {
                ...
                $q.all(promises).then(function (results) {
                    var validated = _.every(results, function (errmsg) {
                        return !errmsg;
                    });
                    if (validated) {
