(function () {
    ...
    angular.module('numaHopApp.service').factory('NumahopAutoCheckService', NumahopAutoCheckService);
    ...
    function NumahopAutoCheckService(gettext, MessageSrvc, AutoCheckSrvc) {
        ...
        function autoCheck(type, identifier) {
            switch (type) {
                case 'facile':
                    checkFacile(identifier);
                    break;
                default:
                    MessageSrvc.addWarn(gettext('Contrôle non reconnu'));
            }
        }
        ...
        function checkFacile(identifier) {
            AutoCheckSrvc.facile({ docUnit: identifier }).$promise.then(function () {
