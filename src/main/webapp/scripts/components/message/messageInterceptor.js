(function () {
    ...
    angular
                ...
                return {
                    ...
                    responseError: function (response) {
                        if (isRestCall(response.config) &&
                            (response.status === 404 || response.status === 405 || response.status === 409 || response.status === 412 || response.status === 417 || response.status === 500)) {
        ...
        .run(function ($rootScope, gettext, MessageSrvc, ErreurSrvc) {
            ...
            $rootScope
                .$on('event:messageInterceptor-httpError',
                    function (event, args) {
                        var alerts;
                        ...
                        if (args.status === 412) {
                            ...
                            if (_.isArray(args.data.errors)) {
                                args.details = _.uniq(_(args.data.errors)
                                    .filter(function (element) {
                                        return !_.isUndefined(element);
                                    }))
                                    .map(function (element) {
                                ...
                                MessageSrvc.addError(gettext("<div>Vous avez tenté de modifier un objet que quelqu'un d'autre a modifié en même temps que vous !</div><div><b>Vos modifications n'ont pas été enregistrées.</b></div><div>Rechargez la page pour mettre à jour les informations, vos saisie seront perdues.</div>"),
                                    args);
                                ...
                                MessageSrvc.addError(gettext("Une erreur inattendue est survenue :<br/>{{statusText}}.<br/>Vous pouvez recharger la page. Si le problème persiste, contactez votre administrateur."),
                                    args);
                            }
