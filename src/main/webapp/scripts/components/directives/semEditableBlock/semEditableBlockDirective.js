(function () {
    ...
    angular.module('numaHopApp.directive').directive('semEditableBlock', function ($compile) {
        return {
            restrict: 'EA',
            scope: false,
                ...
                return {
                    post: function (scope, iElement, iAttrs, editableFormCtrl) {
                        ...
                        if (angular.isDefined(scope.formName) && scope.formName !== '') {
                            var field = iElement.find('sem-editable-field').attr('sem-form', scope.formName);
                        }
