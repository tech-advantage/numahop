(function () {
    ...
    angular.module('numaHopApp.directive')
        .directive('semTable', function ($compile) {
            return {
                restrict: 'A',
                scope: {
                    sortModel: '=semTable',
                ...
                compile: function compile(tElement, tAttrs, transclude) {
                    ...
                    return {
                        pre: function preLink(scope, iElement, iAttrs, controller) {
                            ...
                            iElement.find("th[sem-table-sorton]")
                                .each(function (index, value) {
                                    var _element = angular.element(value);
                                    var sortOn = _element.attr("sem-table-sorton");
                                    var sortIcon = angular.element("<span></span>")
                                        .attr("ng-class", "getSortClass('" + sortOn + "')");
                                    var sortCounter = angular.element("<span></span>")
                                        .addClass("counter")
                                        .attr("ng-show", "sortModel.length > 1")
                                        .attr("ng-bind", "getSortIndex('" + sortOn + "')");
                                    var sortBadge = angular.element("<span></span>")
                                        .addClass("font10")
                                        .attr("ng-show", "sortModel.length > 0 && isSorted('" + sortOn + "')");
                                    sortBadge.append(sortIcon)
                                        .append(sortCounter);

                                    _element.attr("ng-click", "sortOn('" + sortOn + "', sortModel)")
                                        .append(sortBadge);
