(function () {
    ...
    /* Services */
    angular.module('numaHopApp.service').factory('Account', function ($resource) {
        return $resource('api/rest/account', {}, {});
    });

    angular.module('numaHopApp.service').factory('Password', function ($resource) {
        return $resource('api/rest/account/change_password', {}, {});
    });

    angular.module('numaHopApp.service').factory('AuthenticationSharedService', function ($rootScope, $http, authService, Principal, USER_ROLES) {
        var service = {
    ...
    angular.module('numaHopApp.service').factory('LocaleService', function (gettextCatalog, tmhDynamicLocale, $cookies, CONFIGURATION) {
        var currentLocale;
        return {
            changeLocale: function (locale) {
                ...
                $cookies.put('tmhDynamicLocale.locale', locale);
            },
            initLocale: function () {
                var locale = $cookies.get('tmhDynamicLocale.locale');
    ...
    angular.module('numaHopApp.service').factory('VersionCheckerService', function ($http, $q, $rootScope, $timeout) {
        var service = {
            checkNewVersionAvailable: function (version) {
                $http.get('build-date.json?cachebuster=' + moment().format('X')).then(
                    function (response) {
            ...
            getVersion: function () {
                var defer = $q.defer();
                $http.get('/api/rest/numahop?build=true').then(
                    function (response) {
