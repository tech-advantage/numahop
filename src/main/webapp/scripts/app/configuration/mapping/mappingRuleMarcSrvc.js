(function () {
    ...
    angular.module('numaHopApp.service')
        .factory('MappingRuleMarcSrvc', MappingRuleMarcSrvc);
    ...
    function MappingRuleMarcSrvc(MappingRuleCtxMenuSrvc) {
        ...
        function ctxCondition(rule) {
            return [
                ["Existence d'un champ / sous-champ", function ($itemScope, $event) {
        ...
        function ctxExpressionConf(rule) {
            return [
                ...
                ["Sous-champs (regroupements): ajout d'un groupe de sous-champs", function ($itemScope, $event) {
                ...
                ["Sous-champs (regroupements): filtrage des sous-champs", function ($itemScope, $event) {
                    rule.expressionConf = MappingRuleCtxMenuSrvc.wrapWith(rule.expressionConf, $event, "subfieldsFilter", "'<code>'...");
                }],
                ["Sous-champs (regroupements): séparateur par défaut", function ($itemScope, $event) {
        ...
        function ctxLeader(rule, field) {
            return [
                ...
                // }],
                ["Longueur de la partie \"longueur de zone\" (20)", function ($itemScope, $event) {
                    rule[field] = MappingRuleCtxMenuSrvc.replaceWith(rule[field], $event, "(String)leader.getEntryMap()[0]");
                }],
                ["Longueur de la partie \"position du premier caractère\" (21)", function ($itemScope, $event) {
                    rule[field] = MappingRuleCtxMenuSrvc.replaceWith(rule[field], $event, "(String)leader.getEntryMap()[1]");
                }],
                ["Longueur de la partie \"partie relative à l'application\" (22)", function ($itemScope, $event) {
        ...
        function getFields(text, ignorePrefix) {
            var allMatches = [];
            var marcRegexp = ignorePrefix ?
                /([x\d]{3}(?!\d))(?:(?!\d)\$([0-9a-zA-Z]))?/g
                : /(?:\\)([x\d]{3}(?!\d))(?:(?!\d)\$([0-9a-zA-Z]))?/g;
