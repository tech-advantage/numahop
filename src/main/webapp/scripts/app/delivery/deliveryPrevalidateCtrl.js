(function () {
    ...
    angular.module('numaHopApp.controller').controller('DeliveryPrevalidateCtrl', DeliveryPrevalidateCtrl);

    function DeliveryPrevalidateCtrl($routeParams, $scope, DeliverySrvc, codeSrvc, gettextCatalog, MessageSrvc, ModalSrvc, ValidationSrvc, ErreurSrvc) {
        ...
        function deliver() {
            ...
            DeliverySrvc.deliver(params, metadatas)
        ...
        function loadPreDelivery() {
            DeliverySrvc.predeliver(
                { id: $routeParams.id, create_docs: $scope._createDocs },
                function (predelivery) {
                    $scope.predelivery = predelivery;
                    if (predelivery.errors && predelivery.errors.length > 0) {
                        $scope.emptyDelivery =
                            _.filter(predelivery.errors, function (error) {
                                return error.code == 'DELIVERY_NO_MATCHING_PREFIX' || error.code == 'DELIVERY_NO_MASTER_FOUND';
                            }).length > 0;
                        $scope.availableSpaceOnDisk =
                            !_.filter(predelivery.errors, function (error) {
                                return error.code == 'DELIVERY_NOT_ENOUGH_AVAILABLE_SPACE';
                            }).length > 0;
                    }
