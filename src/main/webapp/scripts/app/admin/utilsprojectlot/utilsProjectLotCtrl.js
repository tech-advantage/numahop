(function () {
    ...
    angular.module('numaHopApp.controller').controller('UtilsProjectLotCtrl', UtilsProjectLotCtrl);

    function UtilsProjectLotCtrl($location, $q, $scope, $timeout, DocUnitSrvc, gettext, gettextCatalog, MessageSrvc, LotSrvc, NumaHopInitializationSrvc, TrainSrvc) {
        ...
        function loadOptionsAndFilters() {
            $q.all([NumaHopInitializationSrvc.loadLibraries(), NumaHopInitializationSrvc.loadCompleteProjects(), NumaHopInitializationSrvc.loadCompleteLots()]).then(function (data) {
        ...
        function refreshFilterLists() {
            ...
            NumaHopInitializationSrvc.loadCompleteProjects(librariesIds).then(function (data) {
                $scope.options.projects = data;

                NumaHopInitializationSrvc.loadCompleteLots(librariesIds, projectsIds).then(function (data) {
        ...
        function closeLot() {
            ...
            LotSrvc.closeLot(params, lotsIds).$promise.then(function (res) {
                $scope.results = res;
            });
        }
        ...
        function declotureLot() {
            ...
            LotSrvc.uncloseLot(params, lotsIds).$promise.then(function (res) {
                $scope.results = res;
            });
        }
    }
