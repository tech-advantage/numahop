(function () {
    ...
    angular.module('numaHopApp.controller')
        .controller('UtilsWorkflowCtrl', UtilsWorkflowCtrl);

    function UtilsWorkflowCtrl($location, $q, $scope, $timeout, DocUnitSrvc, gettext, gettextCatalog,
                               WorkflowSrvc, NumaHopInitializationSrvc, TrainSrvc) {
            ...
            NumaHopInitializationSrvc.loadProjects(librariesIds)
                .then(function (data) {
                    ...
                    if (projectsIds.length > 0) {
                            ...
                            TrainSrvc.findByProjectIds(params).$promise
                                .then(function (data) {
        ...
        function validDocWorkflowState() {
            ...
            if (!$scope.filters.docUnits 
                    || $scope.filters.docUnits.length === 0 
                    || !$scope.wkfFilters.validState.identifier ) {
                return;
            }
            var docUnitIds = _.pluck($scope.filters.docUnits, "identifier");
        ...
        function reinitDocWorkflowState() {
            ...
            if (!$scope.filters.docUnits 
                    || $scope.filters.docUnits.length === 0 
                    || !$scope.wkfFilters.reinitState.identifier ) {
                return;
            }
            var docUnitIds = _.pluck($scope.filters.docUnits, "identifier");
