(function () {
    ...
    angular.module('numaHopApp.service')
        .factory('ExportHandlerSrvc', function ($q, FileSaver, gettext, ExportFTPConfigurationSrvc, DocUnitSrvc, MessageSrvc, DateUtils, ModalSrvc, $httpParamSerializer, $http) {
            ...
            function massExport(docUnitId, exportConfig) {
                ...
                if(params) {
                    ...
                    reqPromise = ExportFTPConfigurationSrvc.findByProject({ project: exportConfig.projectId, fullConfig: true }).$promise
                        .then(function(config) { return mapFTPConfigurationToExportParams(docUnitId, config)});
                }

                reqPromise.then(function(exportParams) {
                    //FIXME Use a service instead of $http directly in controller
                    $http.get(DOCUNIT_URL + $httpParamSerializer(exportParams), { responseType: 'arraybuffer' })
