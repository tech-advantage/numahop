(function () {
	...
	angular.module('numaHopApp.controller')
		.controller('ExportAllOperationsCtrl', function ($filter, gettextCatalog, codeSrvc, MessageSrvc, ExportSrvc, FileSaver) {
			...
			self.init = function (parentCtrl) {
				...
				if (angular.isDefined(self.docUnit.iaReports)) {
					...
					_.find(self.docUnit.iaReports, function (report) {
						if (report.status === 'SENT') { // report.dateArchived
			...
			function downloadAIP() {
				...
				ExportSrvc
					.getAip({ docUnit: self.docUnit.identifier })
					.$promise
					.then(function (data) {
			...
			function downloadSIP(status) {
				...
				ExportSrvc
					.getSip({ docUnit: self.docUnit.identifier, cinesStatus: status })
					.$promise
					.then(function (data) {
