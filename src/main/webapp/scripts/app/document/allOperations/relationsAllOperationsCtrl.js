(function () {
    ...
    angular.module('numaHopApp.controller').controller('RelationsAllOperationsCtrl', RelationsAllOperationsCtrl);
    ...
    function RelationsAllOperationsCtrl($location, DocUnitSrvc, gettext, MessageSrvc, ModalSrvc) {
        ...
        function addChild() {
            ...
            ModalSrvc.selectDocUnit(params).then(function (selection) {
                DocUnitSrvc.save({ parent: relCtrl.docUnit.identifier, addchild: true }, _.pluck(selection, 'identifier')).$promise.then(function (data) {
        ...
        function removeChild(child) {
            DocUnitSrvc.save({ parent: relCtrl.docUnit.identifier, child: child.identifier, removechild: true }, {}).$promise.then(function (data) {
                MessageSrvc.addSuccess(gettext("L'unité documentaire {{pgcnId}} {{label}} a été sauvegardée"), relCtrl.docUnit);
        ...
        function addSibling() {
            ...
            ModalSrvc.selectDocUnit(params).then(function (selection) {
                DocUnitSrvc.save({ siblingid: relCtrl.docUnit.identifier, addsibling: true }, _.pluck(selection, 'identifier')).$promise.then(function (data) {
                    MessageSrvc.addSuccess(gettext("L'unité documentaire {{pgcnId}} {{label}} a été sauvegardée"), relCtrl.docUnit);
        ...
        function removeSibling(sibling) {
            DocUnitSrvc.save({ siblingid: relCtrl.docUnit.identifier, removesibling: sibling.identifier }, {}).$promise.then(function (data) {
                MessageSrvc.addSuccess(gettext("L'unité documentaire {{pgcnId}} {{label}} a été sauvegardée"), relCtrl.docUnit);
