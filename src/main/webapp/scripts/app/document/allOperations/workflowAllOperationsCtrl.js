(function () {
    ...
    angular.module('numaHopApp.controller').controller('WorkflowAllOperationsCtrl', WorkflowAllOperationsCtrl);

    function WorkflowAllOperationsCtrl($routeParams, $location, $q, WorkflowSrvc, codeSrvc, gettextCatalog, MessageSrvc, DocUnitSrvc, DateUtils, FileSaver, ExportHandlerSrvc) {
        ...
        function loadWorkflow(identifier) {
            ...
            workflowCtrl.workflow.$promise.then(function (workflow) {
                workflowCtrl.loaded = true;
                _.each(workflow.states, function (state) {
                    state.authorized = false;
                    if (state.status === 'PENDING') {
                        WorkflowSrvc.canProcess({ docUnit: identifier, key: state.key }).$promise.then(function (result) {
                            if (result.value) {
        ...
        function validate(state) {
            var params;
            if (state.key === 'LIVRAISON_DOCUMENT_EN_COURS' || state.key === 'RELIVRAISON_DOCUMENT_EN_COURS') {
                params = {
                    new: true,
                ...
                WorkflowSrvc.process({ docUnitId: workflowCtrl.docUnitId, key: state.key })
        ...
        function afterValidated(key, docId) {
            return typeof workflowCtrl.postValidationTasks[key] === 'function' ? workflowCtrl.postValidationTasks[key](docId) : null;
        }
        ...
        function sortStates(states) {
            ...
            relivStates = _.sortBy(relivStates, function (relivState) {
                return new Date(relivState.startDate);
            }).reverse();
