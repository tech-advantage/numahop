(function () {
    ...
    angular.module('numaHopApp.controller')
        .controller('ImportEditCtrl', ImportEditCtrl);

    function ImportEditCtrl($filter, $http, $httpParamSerializer, $q, $location, $routeParams, $scope, $timeout, codeSrvc, CONFIGURATION,
        CSVMappingSrvc, ErreurSrvc, gettext, gettextCatalog, ImportSrvc, ImportedDocUnitSrvc, ImportReportSrvc, LotSrvc, MappingSrvc,
        MessageSrvc, ModalSrvc, NumaHopInitializationSrvc, Principal, ProjectSrvc, USER_ROLES, WebsocketSrvc, Z3950Srvc) {
        ...
        function loadMappings() {
            var result = editCtrl.isSuperAdmin
                ...
                : MappingSrvc.query({ usable: true, library: editCtrl.library.identifier });
        ...
        function loadCSVMappings() {
            var result = editCtrl.isSuperAdmin
                ...
                : CSVMappingSrvc.query({ usable: true, library: editCtrl.library.identifier });
        ...
        function importZ3950() {
            return ModalSrvc.open("scripts/app/document/z3950/modalZ3950Search.html", {}, "lg", "ModalZ3950SearchCtrl")
                .then(function (z3950record) {
                    var params = {};
                    ImportSrvc.clearParams();
                    // Bibliothèque
                    ...
                    // Import de la notice
                    Z3950Srvc.import(params, z3950record).$promise
                        .then(function (data) {
                            MessageSrvc.addSuccess(gettext("L'import est en cours d'exécution"));
                            $location.search({ id: data.identifier }); // màj url
        ...
        function importDocUnits(report) {
            ...
            ImportSrvc.importReport(params, report).$promise
                .then(function (data) {
                    MessageSrvc.addSuccess(gettext("L'import est en cours d'exécution"));
                    // Synchro du scope parent
                    $scope.$emit("editCtrl:synchro", data);
        ...
        function subscribeImport(report) {
            ...
            subscriberPromise
                ...
                .then(function (s) { editCtrl.subscriber = s; })
                .then(function () {
                    ...
                    if (editCtrl.runningStatus[report.status]) {
                        ...
                        editCtrl.subscrCheckPromise = $timeout(function () {
                            ImportReportSrvc.getStatus({ id: report.identifier }).$promise
                                .then(function (data) {
                                    // Désabonnement
        ...
        function hasImportFile(report, file) {
            if (report.identifier) {
                if (angular.isUndefined(editCtrl.cache.hasImportFiles[report.identifier])) {
                    editCtrl.cache.hasImportFiles[report.identifier] =
                        ImportReportSrvc.get({ id: report.identifier, hasfile: true }).$promise
                            .then(function (data) {
        ...
        function getPage(tableState) {
            ...
            ImportedDocUnitSrvc.get(params).$promise
                .then(function (page) {
        ...
        function checkOaiPmhQuery() {
            ...
            $http.get("api/rest/oaipmh?" + $httpParamSerializer(params))
