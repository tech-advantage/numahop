...
public class DeliveryController extends AbstractRestController {
    ...
    public ResponseEntity<Page<SimpleDeliveryDTO>> search(final HttpServletRequest request,
        ...
        return new ResponseEntity<>(uiDeliveryService.search(search,
                                                             filteredLibraries,
                                                             filteredProjects,
                                                             lots,
                                                             providers,
    ...
    public ResponseEntity<List<AuditDeliveryRevisionDTO>> getDeliveriesForWidget(final HttpServletRequest request,
        ...
        // Chargement
        final List<AuditDeliveryRevisionDTO> revisions =
            uiDeliveryService.getDeliveriesForWidget(fromDate, filteredLibraries, filteredProjects, lots, status, false);
        // Réponse
        return new ResponseEntity<>(revisions, HttpStatus.OK);
    }

    @RequestMapping(method = RequestMethod.GET, params = {"sampledWidget", "from"}, produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    @RolesAllowed({DEL_HAB0})
    public ResponseEntity<List<AuditDeliveryRevisionDTO>> getSampledDeliveriesForWidget(final HttpServletRequest request,
        ...
        // Chargement
        final List<AuditDeliveryRevisionDTO> revisions =
            uiDeliveryService.getDeliveriesForWidget(fromDate, filteredLibraries, filteredProjects, lots, status, true);
        // Réponse
        return new ResponseEntity<>(revisions, HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> deliver(final HttpServletRequest request,
                                     ...
                                     @RequestParam(value = "prefixToExclude", required = false) final List<String> prefixToExclude) throws
    ...
    public ResponseEntity<PreDeliveryDTO> predeliver(final HttpServletRequest request,
                                                     @PathVariable final String id,
                                                     @RequestParam(name = "create_docs", required = false, defaultValue = "false")
    ...
    @Timed
    @RolesAllowed(DEL_HAB0)
    public void generateSlipLotPDF(final HttpServletRequest request, final HttpServletResponse response, @PathVariable final String id) throws
                                                                                                                                        PgcnTechnicalException {
    ...
    public ResponseEntity<Collection<SimpleDeliveryDTO>> findAllSimpleDTO() {
        ...
        final Collection<SimpleDeliveryDTO> delivsDTO =
            delivs.stream().map(DeliveryMapper.INSTANCE::deliveryToSimpleDeliveryDTO).collect(Collectors.toList());
    ...
    @RequestMapping(method = RequestMethod.GET, params = {"docUnit", "latest"}, produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    @RolesAllowed({DEL_HAB0})
    ...
    @Timed
    public ResponseEntity<DeliveryDTO> detachDigitalDoc(@PathVariable final String id, @RequestBody final DigitalDocumentDTO docToDetach) throws
                                                                                                                                          PgcnException {
        // Droits d'accès
        if (!accessHelper.checkDelivery(id)) {
            return new ResponseEntity<>(HttpStatus.FORBIDDEN);
    ...
    public ResponseEntity<List<String>> getDeliveryProgress(@PathVariable final String id,
                                                            @RequestParam(value = "digitalId") final String digitalId) {
    ...
    /**
     * Stocke le % d'avancement de la livraison d'un doc.
     *
     * @param id
     * @param datas
     * @return
     ...
     */
    @RolesAllowed({DEL_HAB0})
    @RequestMapping(value = "/{id}", method = RequestMethod.POST, params = {"delivprogress"}, produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<?> setDeliveryProgress(@PathVariable final String id, @RequestBody final List<String> datas) throws PgcnException {

        if (datas == null || datas.size() != 2) {
            return new ResponseEntity<>(HttpStatus.EXPECTATION_FAILED);
        }
        final String digitalId = datas.get(0);
        if (StringUtils.isNotBlank(digitalId)) {
            final Map<String, String> values = DeliveryController.PROGRESS_VALUES.get(id);
            if (values == null) {
                final Map<String, String> vals = new HashMap<>();
                vals.put(digitalId, datas.get(1));
                DeliveryController.PROGRESS_VALUES.put(id, vals);
            } else {
                values.put(digitalId, datas.get(1));
            }
        }
        return new ResponseEntity<>(HttpStatus.OK);
    }
