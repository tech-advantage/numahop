...
public class ConcatWithSepFormatter extends CustomScript {
    ...
    public String format(final DataField field) {
        ...
        final List<Subfield> subfields = field.getSubfields().stream().filter(sub -> ArrayUtils.contains(codes, sub.getCode())).collect(Collectors.toList());
        ...
        for (final Subfield subfield : subfields) {
            if (lastTag != null) {
                buf.append(lastTag == subfield.getCode() ? sepInner
                                                         : sepOuter);
            }
