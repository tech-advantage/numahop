...
public class ImportMarcService extends AbstractImportService {
    ...
    public void importMarcAsync(final List<File> importFiles,
        ...
        try {
            ...
            for (final File importFile : importFiles) {
                report = preimportFile(importFile, fileFormat, dataEncoding, mappingId, mappingChildId, parentKeyExpr, report, archivable, distributable);
            }
    ...
    private ImportReport preimportFile(final File importFile,
        ...
        if (fileFormat == FileFormat.MARC) {
            final String encoding = dataEncoding == DataEncoding.UTF_8 ? StandardCharsets.UTF_8.name() : dataEncoding == DataEncoding.ISO_8859_1 ? StandardCharsets.ISO_8859_1
                                                                                                                                                                              .name()
                                                 : null;
        ...
        // MARC JSON
            ...
            return importMarcRecords(importFile, dataEncoding, mappingId, mappingChildId, parentKeyExpr, MarcJsonReader::new, report, archivable, distributable);
        }
        // MARC XML
        else if (fileFormat == FileFormat.MARCXML) {
            return importMarcRecords(importFile, dataEncoding, mappingId, mappingChildId, parentKeyExpr, MarcXmlReader::new, report, archivable, distributable);
        }
        // Non géré
        else {
    ...
    private ImportReport importRecord(final MarcReader reader,
        ...
        final CompiledMappingRule joinScript = StringUtils.isNotBlank(parentKeyExpr) ? mappingEvaluationService.compileMapping(parentKeyExpr, charConverter)
                                                                                     : null;
        ...
        new TransactionalJobRunner<>(new MarcRecordIterator(reader), transactionService)
                                                                                        ...
                                                                                        .forEach((record) -> {
                                                                                            try {
                                                                                                ...
                                                                                                final List<DocUnitWrapper> docUnits = convertService.convert(record,
                                                                                                                                                             charConverter,
                                                                                                                                                             compiledMapping,
                                                                                                                                                             joinScript,
                                                                                                                                                             propertyTypes);
                                                                                                ...
                                                                                                if (mappingChild != null && !docUnits.isEmpty()) {
                                                                                                    ...
                                                                                                    final List<DocUnitWrapper> childrenDocUnits = convertService.convert(record,
                                                                                                                                                                         charConverter,
                                                                                                                                                                         compiledMappingChild,
                                                                                                                                                                         null,
                                                                                                                                                                         propertyTypes);
                                                                                                ...
                                                                                                // regroupement
                                                                                                final String groupCode = docUnits.size() > 1 ? record.getControlNumber()
                                                                                                                                             : null;
