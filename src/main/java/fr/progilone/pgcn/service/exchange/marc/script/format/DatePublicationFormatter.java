...
public class DatePublicationFormatter extends CustomScript {
    ...
    public String getConfigScript() {
        return "def " + SCRIPT_NAME + "Filter = {\n"
               + "      String code -> script." + getCode() + ".setSubfield((char)code)\n"
               + "}\n"
               + "def " + SCRIPT_NAME + "PositionDebut = {\n"
               + "      int start -> script." + getCode() + ".setDateStartPosition(start)\n"
               + "}\n"
               + "def " + SCRIPT_NAME + "PositionFin = {\n"
               + "      int end -> script." + getCode() + ".setDateEndPosition(end)\n"
               + "}\n"
    ...
    public String format(final DataField field) {
        ...
        if(!subfieldForDate.isPresent()) return null;
        ...
        if(baseDateValueData == null || baseDateValueData.length() <= endPosition) return null;
        // le type de publication doit aussi faire partie de la chaîne
        if(baseDateValueData.length() <= typePosition) return null;
        ...
        switch(typeDate) {
            ...
            case 'f':
            case 'b':
            case 'a' : sb.append(baseDateValueData.substring(startPosition, startPosition + 4))
            ...
            case 'e': sb.append(baseDateValueData.substring(startPosition, startPosition + 4))
                        .append(SEPARATOR_OTHER_DATE)
                        .append(baseDateValueData.substring(startPosition + 4, endPosition + 1));
            break;
            // Blancs sur les positions suivantes
            case 'd':
            case 'c': sb.append(baseDateValueData.substring(startPosition, startPosition + 4));
            break;
            case 'j': sb.append(baseDateValueData.substring(startPosition, startPosition + 4))
