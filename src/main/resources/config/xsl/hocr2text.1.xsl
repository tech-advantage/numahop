    ...
    <xsl:template match="*">
        <xsl:choose>
            <xsl:when test="local-name() = 'strong'">
                <xsl:call-template name='ansi'>
                    <xsl:with-param name='sgr'>1m</xsl:with-param>
                </xsl:call-template>
            </xsl:when>
            <xsl:when test="local-name() = 'em'">
                <xsl:call-template name='ansi'>
                    <xsl:with-param name='sgr'>3m</xsl:with-param>
                </xsl:call-template>
            </xsl:when>
        </xsl:choose>
        <xsl:apply-templates/>
        <xsl:call-template name='ansi'>
            <xsl:with-param name='sgr'>0m</xsl:with-param>
        </xsl:call-template>
        <xsl:text>&#x20;</xsl:text>
    </xsl:template>
